...
st.markdown(
    ...
    .bc-hero h1 { color: white; margin: 0 0 6px 0; font-size: 2rem; }
    .bc-hero p { color: rgba(255,255,255,0.92); margin: 0; font-size: 1.02rem; }
    .bc-badges { margin-top: 14px; display: flex; gap: 8px; flex-wrap: wrap; }
    .bc-badge {
    ...
    /* Segmented control (source toggle) */
    div[role="radiogroup"] {
    ...
    .bc-score-number { font-size: 3.2rem; font-weight: 700; color: #0e7490; line-height: 1; }
    .bc-score-max { font-size: 1.1rem; color: #64748b; font-weight: 500; }
    .bc-score-assessment {
    ...
    /* Footer */
    .bc-footer { text-align: center; color: #94a3b8; font-size: 0.8rem; margin-top: 40px; }
    </style>
    """,
...
with st.sidebar:
    ...
    if not _sessions:
        st.caption("No sessions scored yet. Your first result will show up here.")
    else:
        st.caption(f"{len(_sessions)} figure{'s' if len(_sessions) != 1 else ''} scored")
        ...
        for _s in _sessions[:25]:
            ...
            with st.expander(f"{_s['swimmer_id']} — {_score_str} ({_s['timestamp']})"):
                ...
                if st.button("🗑️ Delete", key=f"del_{_s['id']}", use_container_width=True):
...
st.markdown('<div class="bc-section-label">Camera Source</div>', unsafe_allow_html=True)
source = st.radio(
...
# NOTE: there are exactly 3 valid tracking+scoring combinations —
# Walticam, Above-only, and Above+Below. "Below only" is intentionally
# NOT offered: the base/height score is measured as how far the ankles
# rise above the water surface, which is physically invisible from an
# underwater-only camera — there's no score to compute without an
# above-water video, so it's not presented as if there were one.
above_water = True
below_water = False
if source == "Above / Below":
    ...
    if below_water:
        st.caption(
            "ℹ️ The underwater video adds coaching-only feedback (bent "
            "knee, back layout depth) — the official score always comes "
            "from the above-water footage."
        )
...
def _assessment_style(score):
    """Returns (label, color) for the score badge."""
...
def show_score(above_kalman_csv, below_kalman_csv=None, name="figure"):
    ...
    st.markdown('<div class="bc-section-label">Result</div>', unsafe_allow_html=True)

    score = result["score"]
    assess, color, bg = _assessment_style(score)

    st.markdown(
        f"""
        <div class="bc-score-card">
            <div style="color:#64748b; font-size:0.85rem; font-weight:600; letter-spacing:0.04em;">
                {name.upper() if name else "FIGURE"}
            </div>
            <div><span class="bc-score-number">{score:.2f}</span><span class="bc-score-max"> / 10.0</span></div>
            <div class="bc-score-assessment" style="color:{color}; background:{bg};">{assess}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if source == "Walticam":
    ...
    if uploaded_file is not None:
        ...
        elif st.button("▶️ Process Video", type="primary", use_container_width=True):
...
# ============================================================================
    ...
    if any_selected and ready and (above_file is not None or below_file is not None):
        ...
        elif st.button("▶️ Process Video(s)", type="primary", use_container_width=True):
            with tempfile.TemporaryDirectory() as tmp_dir:
...
with st.expander("ℹ️ About this tracker"):
    st.markdown(
        """
        This tool uses **RTMPose** (via `rtmlib`) for pose detection, with three modes:
        ...
        - **Above / Below**: separate above-water and/or underwater videos,
          each run through a dedicated tracker with:
            ...
            - Waterline detected from shoulder + hip position (nose excluded,
              since it sits above the true surface even in a good float)
            - Validated swimmer locking — anatomy + position checks, with a
              periodic re-check requiring two consecutive disagreements
              before switching who's tracked
            - Multi-level smoothing and Kalman filtering
            - A synthesized `mid_spine` point (underwater only) for back-
        ...
        **deductions** for alignment, backpike, leg/ankle extension, back
        roundness, travel, and unroll speed — blended 70% absolute (fixed
        thresholds) / 30% relative (how this figure compares to others
        scored in the same batch).

        Output includes an annotated video with the detected skeleton and
        waterline, plus a CSV of per-frame joint positions for further
        analysis, and every scored figure is saved to session history
        (see the sidebar) for later reference.
        """
    )

st.markdown(
    '<div class="bc-footer">Barracuda Tracker — RTMPose + Kalman filtering + FINA-aligned scoring</div>',
    unsafe_allow_html=True,
)
