...
def detect_waterline_from_poses(video_path, pose_tracker_fn):
    """Averages head/shoulder/hip position across the first 100 frames of
    horizontal-swimmer detections to find the waterline."""
    ...
    for frame_num in range(max_search_frame):
        ...
        for kps, sc in zip(keypoints, scores):
            if not _is_horizontal_coco(kps, sc, h_frame, w_frame):
                continue
            if sc[0] > 0.5:
                head_positions.append(kps[0][1] / h_frame)
            if sc[5] > 0.4 and sc[6] > 0.4:
                shoulder_positions.append((kps[5][1] + kps[6][1]) / 2 / h_frame)
            if sc[11] > 0.4 and sc[12] > 0.4:
                hip_positions.append((kps[11][1] + kps[12][1]) / 2 / h_frame)
    cap.release()
    ...
    signals = [v for v in [_clean(head_positions), _clean(shoulder_positions), _clean(hip_positions)] if v is not None]
